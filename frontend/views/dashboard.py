                ...
                with col_msgs:
                    ...
                    for m in m_list:
                        ...
                        <div class="message-container">
                            ...
                            <div class="message-bubble {cls}">{m['content']}</div>
                        </div>
                        """, unsafe_allow_html=True)
        ...
        with st.expander("?? Add Strategic Constraint / User Preference", expanded=False):
            ...
            if st.button("Save Preference"):
                if pref_key and pref_val:
                    try:
                        res = requests.post(f"{API_URL}/memory/add", json={
                            "key": pref_key,
                            "value": pref_val,
                            "category": "preference"
                        })
        ...
        if search_q:
            try:
                ...
                if res.status_code == 200:
                    ...
                    for it in items:
                        col_text, col_del = st.columns([5, 1.2])
                        with col_text:
                            st.markdown(f"""
                        ...
                        with col_del:
                            st.write("")
                            st.write("")
                            if st.button("Delete", key=f"del_{it['id']}", use_container_width=True):
                                del_res = requests.delete(f"{API_URL}/memory/delete/{it['id']}")
